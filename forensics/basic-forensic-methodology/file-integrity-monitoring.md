...
基线包括拍摄系统的某些部分的快照，以**将其与未来状态进行比较以突出显示更改**。
...
例如，您可以计算和存储文件系统文件的每个文件的哈希，以便能够确定修改了哪些文件。\ \ \ \
...
这也可以通过创建的用户帐户，运行的流程，运行服务以及任何其他应该更改或根本不应该更改的内容来完成。
...
文件完整性监视是用于保护IT基础架构和业务数据的最强大的技术之一，以应对各种已知和未知威胁。
...
目的是生成要监视的所有文件**的基线，然后定期** ** ** ** ** **这些文件是否可能**更改**（在内容，属性，元数据中， ETC。）。
...
1 \。 **基线比较，**其中一个或多个文件属性将被捕获或计算并存储为基准，以将来可以比较。 但是，这可以像文件的时间和日期一样简单，但是，由于可以轻松地欺骗此数据，因此通常使用更值得信赖的方法。 这可能包括定期评估受监视文件的加密校验和（例如，使用MD5或SHA-2哈希算法），然后将结果与先前计算的校验和比较。
...
2 \。 **实时更改通知**，通常在访问或修改文件时会标记的操作系统内部或作为操作系统内核的扩展。

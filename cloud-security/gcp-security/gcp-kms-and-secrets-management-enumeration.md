...
[云密钥管理服务]（https://cloud.google.com/kms/docs/）是存储加密密钥的存储库，例如用来** eccrypt和解密敏感文件**的存储库。 单个键存储在钥匙环中，并且可以在任一级上应用颗粒权限。

Having **permissions to list the keys** this is how you can access them:

拥有**列出密钥的权限**这就是您可以访问它们的方式：
...
Google [Secrets Management]（https://cloud.google.com/solutions/secrets-management/）是一种用于存储密码，API键，证书和其他敏感数据的保险库解决方案。 截至撰写本文时，它目前正在Beta中。
...
请注意，更改秘密条目将创建一个新版本，因此值得将上述命令中的1`更改为`2'等。
